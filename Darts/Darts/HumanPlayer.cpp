...
HumanPlayer::HumanPlayer(int16_t score, std::string name) : _score(score), _name(name), _winCounter(0), _busted(false)
{
	std::cout << "HumanPlayer constructor called!\n";
}

HumanPlayer::~HumanPlayer()
{
	std::cout << "HumanPlayer destructor called!\n";
}


void HumanPlayer::SetScore(uint16_t score)
{
	_score = score;
}

void HumanPlayer::SubtractScore(int16_t points)
{
	_score -= points;
}

int16_t HumanPlayer::GetScore()
{
	return _score;
}

bool HumanPlayer::IsOdd(int16_t n)
{
	return (n % 2 != 0);
}

bool HumanPlayer::IsEven(int16_t n)
{
	return (n % 2 == 0);
}
...
void HumanPlayer::SetBusted(bool value)
{
	_busted = value;
}

bool HumanPlayer::GetBusted()
{
	return _busted;
}
...
void HumanPlayer::SetName(std::string name)
{
	_name = name;
}

void HumanPlayer::IncrementWinCounter() {
	_winCounter++;
}

uint16_t HumanPlayer::GetWinCounter() {
	return _winCounter;
}

void HumanPlayer::ThrowSingle(uint16_t d, Board* board)
{
	//subtract result of throwing for single d
	//d is the score that the HumanPlayer is aiming for
...
void HumanPlayer::ThrowDouble(uint16_t d, Board* board)
{
	//subtract result of throwing for single d
	//d is the score that the HumanPlayer is aiming for
...
void HumanPlayer::ThrowTriple(uint16_t d, Board* board)
{
	//subtract result of throwing for single d
	//d is the score that the HumanPlayer is aiming for
...
bool HumanPlayer::ThrowBullPercentage(uint16_t percentage)
{
...
void HumanPlayer::AimTon80(Board* board) //Aim to score 180
{
...
void HumanPlayer::AimCheckout141(Board* board) //Aim to score 141
{
	ThrowTriple(20, board);
...
void HumanPlayer::AimThree167s(Board* board) //Aim to score 167
{
	ThrowTriple(20, board);
	ThrowTriple(19, board);
...
//void HumanPlayer::NineDartFinish1(Board* board)
//{
//    AimTon80(board);
//	AimTon80(board);
...
//}
//
//void HumanPlayer::NineDartFinish2(Board* board)
//{
//	AimTon80(board);
//	AimTon80(board);
//	AimThree167s(board);
